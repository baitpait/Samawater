...
## [2026-08-24 18:05] — جلسة تشغيل: حسب الطلب، تصدير الفلاتر، أيام الاشتراك، واجهة الجدول

- **الهدف:** استعادة موثوقية قائمة التسليم وتقارير الفلاتر على الإنتاج (`sama.baitpait.space`)، وتصحيح دورة نوع «غير محدود»، مع توثيق قرارات التشغيل.

- **التغييرات:**

  ### أ) تسليم حسب الطلب
  - زر الفلاتر يحفظ `delivery_on_demand` ويظهر المشترك في قائمة التسليم / الإدخال الجماعي دون شرط الأيام.
  - إصلاح انهيار الصفوف: `toBase()` + `collect()->unique(client_id)` بدل `Eloquent\Collection::merge` بمفتاح `id = null`.

  ### ب) تصدير صفحة الفلاتر (`/admin/reports/filters`)
  - PDF: `Output(STRING_RETURN)` بدل `'I'` (كان جسم الاستجابة 0 بايت).
  - PDF على PHP-FPM: رفع `memory_limit` مؤقتاً إلى 256M + `simpleTables`/`packTableData` (كان Fatal عند 64MB في `mpdf/Otl.php` → HTTP 500).
  - Excel/PDF: أعمدة مطابقة للجدول (دين، طريقة تعامل، حسب الطلب، ملاحظات، رصيد قوارير رقم…).
  - حدود صفوف الجدول: `#111111` لسهولة المسح البصري.

  ### ج) أنواع الاشتراك / قائمة المستحقين
  - `distribution_days` = عدد الأيام بين التسليمات؛ الـ View `v_clients_due_by_type_days_ids` تستخدم `HAVING days_since_last_delivery >= distribution_days` وتشترط وجود تسليم سابق (`JOIN deliveries`).
  - نوع **«غير محدود»**: من `0` → **`30`** (هجرة بيانات آمنة + افتراضي الاستيراد) حتى لا يُعتبر كل من له أي تسليم مستحقاً دائماً.

  ### د) سابقاً في نفس اليوم (منشور)
  - عمود رصيد القوارير في الفلاتر؛ مجموع الدفعات في كشف الحساب.

- **الأدوات:** لا مكتبات جديدة (mPDF الموجود مسبقاً).

- **الكوميتات على `main` / السيرفر:**
  - `0b119de` — مجموع الدفعات في كشف الحساب
  - `c30de9d` — عمود رصيد القوارير في الفلاتر
  - `74b4eb1` — UX زر حسب الطلب وظهور القائمة
  - `4bf55a0` — إصلاح دمج حسب الطلب في قائمة التسليم
  - `8db757e` — إصلاح PDF + مطابقة أعمدة Excel
  - `91f5166` — رفع ذاكرة تصدير PDF تحت PHP-FPM
  - `ee198ff` — حدود جدول الفلاتر غامقة
  - `ac46435` — أيام «غير محدود» = 30

- **تنبيه (يدوي بعد السحب):**
  ```bash
  ...
  ```
  - ملاحظة تشغيلية: جدول `client_statuses` على الإنتاج كان فارغاً (لا يمنع حسب الطلب).
  - قائمة المستحقين من الـ View تبقى ضيقة إن لم يمرّ ≥ أيام النوع منذ آخر تسليم، أو إن لم يوجد تسليم سابق؛ مسار «حسب الطلب» منفصل.

- **Plan B:** بديل PDF ثقيل: تصدير CSV فقط أو توليد PDF خلفي عبر Queue بحد ذاكرة أعلى.
