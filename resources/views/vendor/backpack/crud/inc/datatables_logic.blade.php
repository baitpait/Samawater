 ...
 @endphp
  ...
  {{-- DATA TABLES SCRIPT --}}
  {{-- DataTables assets are loaded locally via configbackpackui.php --}}
  {{-- DataTables assets are loaded locally via configbackpackui.php --}}
  ...
  <script>
    ...
    window.crud = {
      ...
      dataTableConfiguration: {
        ...
        responsive: {
            details: {
                // NOTE: we set this right before DataTable init (after plugins load)
                // to avoid "Cannot read properties of undefined (reading 'Responsive')"
                display: null,
                type: 'none',
                target: '.dtr-control',
  ...
  <script type="text/javascript">
    // Wait for all scripts to load before initializing DataTables
    (function() {
      var attempts = 0;
      var maxAttempts = 200; // Increased to 200 attempts (max 20s)
      var retryInterval = 100; // 100ms between attempts
      ...
      function checkDataTablesReady() {
        ...
        // Check if DataTables plugin is available
        var hasDataTable = $.fn && (
        ...
        // Check if Responsive is available if needed
        @if ($crud->getResponsiveTable())
      ...
      function tryInitialize() {
        ...
        if (!checkDataTablesReady()) {
          if (attempts < maxAttempts) {
            return setTimeout(tryInitialize, retryInterval);
          }
          console.error('DataTable is not available after ' + (maxAttempts * retryInterval / 1000) + ' seconds.');
          
          // Diagnostic info
          var diag = {
            jQuery: typeof window.jQuery !== 'undefined',
            $: typeof window.$ !== 'undefined',
            '$.fn': typeof window.jQuery !== 'undefined' && typeof window.jQuery.fn !== 'undefined',
            '$.fn.DataTable': typeof window.jQuery !== 'undefined' && typeof window.jQuery.fn !== 'undefined' ? typeof window.jQuery.fn.DataTable : 'N/A',
            '$.fn.dataTable': typeof window.jQuery !== 'undefined' && typeof window.jQuery.fn !== 'undefined' ? typeof window.jQuery.fn.dataTable : 'N/A',
            'Responsive': typeof window.jQuery !== 'undefined' && typeof window.jQuery.fn !== 'undefined' && window.jQuery.fn.dataTable ? typeof window.jQuery.fn.dataTable.Responsive : 'N/A'
          };
          console.table(diag);
          return;
        }

        // All checks passed, initialize DataTables
      ...
      function initTableAndBindings() {
        // Make sure Responsive modal display is set ONLY after plugins are loaded.
        // This prevents "Responsive is undefined" before DataTables loads.
        if (window.crud && window.crud.dataTableConfiguration && window.crud.dataTableConfiguration.responsive && window.crud.dataTableConfiguration.responsive.details) {
          var details = window.crud.dataTableConfiguration.responsive.details;
          if (details.display === null &&
              $.fn && $.fn.dataTable &&
